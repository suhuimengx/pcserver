...
serProcessor = SerialProcessor("COM14", 115200)
'''
数字孪生端websocket服务创建
'''
app = Flask(__name__)
...
def QueryUniCloud(url_count, url_doc):
    ...
    while True:
        ...
        if current_count > init_count:
            ...
            # 插入实时订单
            route_array[car_id] = insert(req, route_index[car_id], route_array[car_id], dataDict)
            print(route_array[car_id], dataDict, route_index[car_id])
        time.sleep(1.2)
...
def Process_Car(car_id):
    ...
    global serProcessor, dataDict
    # 插入获取车辆初始路线
    route_array[car_id] = insert([4,6], route_index[car_id], route_array[car_id], dataDict)
    ...
    while True:
        if route_index[car_id] < len(route_array[car_id]) - 1:
            # 处理动态插入bug
            if route_array[car_id][route_index[car_id]][1] != start[car_id][1]:
