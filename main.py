...
def QueryUniCloud(url_count, url_doc):
    ...
    while True:
        ...
        if current_count > init_count:
            ...
            route_array[car_id] = [[1,0]] + insert(req, route_index[car_id], route_array_temp, dataDict_list[car_id])
            # 同步本车路径
            #将route_array中小于7的元素给routearray_send
            routearray_send = []
            for i in range(len(route_array[car_id])):
                if route_array[car_id][i][0] < 7:
                    routearray_send.append(route_array[car_id][i][0])
            print("routearray_send:",routearray_send)
            socketio.emit('send_message_routearray', {"routearray": routearray_send})
...
def Process_Car(car_id):
    ...
    for i in range(len(route_array[car_id])):
        if route_array[car_id][i][0] < 7:
            routearray_send.append(route_array[car_id][i][0])
    print("routearray_send:",routearray_send)
    socketio.emit('send_message_routearray', {
        "routearray": routearray_send,
    })
